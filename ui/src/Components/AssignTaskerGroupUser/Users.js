...
const useStyles = makeStyles((theme) => ({
  ...
  const Users =({ setSelectedUserId })=>{
    ...
    return(
        ...
        { groupList.map( ele => {
            return <MenuItem style={{ backgroundColor: '#040147'}} key={`${ele.id} ${ele.name}`} value={ele.id}><Typography style={{ color: '#ffffff' }}>{ele.user_name}</Typography></MenuItem>
        })}
        </Select>
      </FormControl>
    )
